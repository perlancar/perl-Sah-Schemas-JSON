package Data::Sah::Filter::perl::JSON::check_decode_array;
...
sub meta {
    +{
        v => 1,
        summary => 'Check that value can be decoded as JSON and it is an array',
        might_fail => 1,
        examples => [
            {value=>'', valid=>0, summary=>'Empty string is not valid JSON'},
            {value=>'null', valid=>0, summary=>'Valid JSON but not an array'},
            {value=>'[1,2,3]'},
...
sub filter {
    ...
    $res->{expr_filter} = join(
        "",
        "do { my \$tmp = $dt; my (\$decoded, \$ref); eval { \$decoded = JSON::MaybeXS->new->allow_nonref->decode(\$tmp); \$ref = ref \$decoded }; \$@ ? [\"String is not a valid JSON: \$@\", \$tmp] : \$ref && \$ref eq 'ARRAY' ? [undef, \$tmp] : [\"String is a valid JSON but not an encoded array\", \$tmp] }",
    );
...
the additional check that the decoded value is an array.
